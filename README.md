...
├── README.md                     你正在看的这份
├── assets/
│   ├── app.css                   帝国理工蓝主题，含响应式栅格
│   ├── config.js                 品种、新闻频道词典、全球市场面板、日历规则 —— 改配置只改这里
│   ├── data.js                   数据层：四个源适配器 + 缓存 + 降级
...
└── tools/
    ...
    ├── ai_digest.py              DeepSeek 复盘，见第 6 节
    ├── cloudflare_worker.js      可选的按需解读代理
    ├── publish.sh                一键：更新数据 → 提交 → 推送
    └── e2e_check.mjs             39 项无头浏览器交互自检
```
...
| A 股 / 港股指数 + 60 日趋势 | 腾讯 `qt.gtimg.cn` + `web.ifzq.gtimg.cn` | ✅ `ACAO: *` | 上证/沪深300/中证500/创业板/恒生/恒生科技/国企 |
| 美债 2Y/10Y/30Y、美元指数、美股、VIX、金铜银 | **CNBC** `quote.cnbc.com` | ✅ `ACAO: *` | **一个请求全部拿到** |
...
| 数据源给到什么 | 画什么 | 例子 |
|---|---|---|
| 日线开高低收（腾讯） | 蜡烛图 + MA5 / MA10 + 成交量 + 区间统计 | 上证 / 中证500 / 恒生 / 标普 / 纳斯达克 / 道琼斯 |
| 只有收盘价（本地库 / 汇率库 / 东财仓单） | 折线 + 面积（不假装有开高低） | SC原油 / 布伦特 / WTI / 石脑油 / USDCNY / PTA 仓单 |
| 只有当日快照 | 只给数值，并明说「没有可画的历史序列」 | 美债 2/10/30Y、2s10s、美元指数、VIX、COMEX 金铜银 |
...
| **输入哈希缓存** | 整段上下文算 sha256 存进产物。数据没变直接复用上次结论，**一次 API 都不打** |
| **一天最多一次 + 异动闸门** | `--anomaly 2` 让它只在真有品种涨跌超过 2% 时才调用 |
| **固定 system prompt** | DeepSeek 有磁盘前缀缓存，固定前缀按 cache-hit 计费，约为未命中的 **1/50** |
...
**一次调用约 865 输入 + 最多 400 输出，粗算 $0.00012。一天一次，一个月不到 3 分钱。**
真账单以 DeepSeek 官方为准——它 2026 年调过价且改成了峰谷定价，所以脚本不硬编码费率，
只报 API 返回的真实 `usage`，费率表在 `tools/ai_digest.py` 顶部可改。
...
```bash
python3 tools/ai_digest.py                      # 一天一次
...
产物写到 `data/ai_digest.json`，页面底部会自动显示。没有这个文件时整个区块隐藏，不会报错。
...
cd polyester-desk
python3 -m http.server 8791 &
node tools/e2e_check.mjs http://127.0.0.1:8791/
```

用无头 Chrome 真跑 39 项：链条节点、热力格子、K 线取数、价差与迷你走势、血统行、
日历倒计时与进度条、情报流条目与高亮、全球市场分组与瓦片、2s10s 利差、跑马灯、
数据新鲜度、AI 区块降级、**配色断言（选中态必须是 `rgb(0,62,116)`，全站不得残留琥珀色）**、
点链条联动、终端指令、频道过滤、60 分钟线时间轴，最后打印所有 console 的 error / warning。
